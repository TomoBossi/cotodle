...
function drawGuesses(s = scaleFactor, x = width / 2, y = scaleFactor * 520) {
  ...
  for (let i = 0; i < numGuesses; i++) {
    ...
    if (loadingOpacity && i < guessResults.length) {
      ...
      fill(
        lastGuess
          ? color(255, 0, 0, loadingOpacity * 255)
          : color(150, loadingOpacity * 255)
      );
      rect(x, y + i * s * 60, s * 310, s * 50, s * 6);
      ...
      switch (guessResults[i] / (abs(guessResults[i]) || 1)) {
        case 0:
          line(
            guessResult.pos.x - s * 7.5,
            guessResult.pos.y - s / 2,
            guessResult.pos.x - s * 5,
            guessResult.pos.y + s * 4
          );
          line(
            guessResult.pos.x - s * 5,
            guessResult.pos.y + s * 4,
            guessResult.pos.x + s * 12,
            guessResult.pos.y - s * 3
          );
          break;
        case 1:
          line(
            guessResult.pos.x,
            guessResult.pos.y - s * 7,
            guessResult.pos.x,
            guessResult.pos.y + s * 7
          );
          line(
      ...
      enterGuess = {
        ...
        enabled: data && !gameEnded && guess < upperBound && guess > lowerBound,
      };
      ...
      strokeWeight(2.5 * s);
      line(
        enterGuess.pos.x - s * 15,
        enterGuess.pos.y + s * 3,
        enterGuess.pos.x + s * 15,
        enterGuess.pos.y + s * 3
      );
      line(
        enterGuess.pos.x + s * 15,
        enterGuess.pos.y - s * 4,
        enterGuess.pos.x + s * 15,
        enterGuess.pos.y + s * 3
      );
      line(
        enterGuess.pos.x - s * 15,
        enterGuess.pos.y + s * 3,
        enterGuess.pos.x - 10 * s,
        enterGuess.pos.y + s * 4 + s * 3
      );
      line(
        enterGuess.pos.x - s * 15,
        enterGuess.pos.y + s * 3,
...
function drawButtons(s = scaleFactor, x = width / 2, y = scaleFactor * 760) {
  ...
  for (let i = -1; i < 10; i++) {
    ...
    if (buttonsHighlightOpacity[i + 1]) {
      ...
      rect(buttons[i].pos.x, y, buttons[i].width, buttons[i].height, s * 5);
      buttonsHighlightOpacity[i + 1] = max(
        buttonsHighlightOpacity[i + 1] - 0.1,
        0
      );
    }
    ...
    if (buttons[i].enabled) {
      stroke(
        150 - loadingOpacity * 110,
        (1 - buttonsHighlightOpacity[i + 1]) * 255
      );
      fill(
        150 - loadingOpacity * 110,
        (1 - buttonsHighlightOpacity[i + 1]) * 255
...
function drawInstruction(
  ...
  if (unloadingOpacity) {
    ...
    text(
      `Adiviná el precio del producto (${
    ...
    const gameWonMessage =
      currentDay in easterEgg
        ? easterEgg[currentDay].gameWonMessage
        : `Ganaste! El precio era $${formatPrice(price)} (´㉨\`)`;
    text(gameWonMessage, x, y);
  } else {
    const gameLostMessage =
      currentDay in easterEgg
        ? easterEgg[currentDay].gameLostMessage
        : `Perdiste! El precio era $${formatPrice(price)} (´㉨\`)`;
    text(gameLostMessage, x, y);
  }
}

function drawImgZoom(
  s = scaleFactor * 0.1,
  x = width / 2 + scaleFactor * 132.5,
  y = scaleFactor * 122
) {
  if (loadingOpacity) {
    imgZoom = {
    ...
    imgLink.position(
      imgZoom.pos.x - imgZoom.width / 2,
      imgZoom.pos.y - imgZoom.height / 2
    );
    imgLink.size(imgZoom.width, imgZoom.height);
    stroke(
      mouseInside(imgZoom)
        ? color(255, 0, 0, loadingOpacity * 255)
        : color(200, loadingOpacity * 255)
    );
    strokeWeight(s * 20);
    noFill();
...
function drawDisplayName(
  ...
  if (unloadingOpacity) {
    ...
    const name =
      currentDay in easterEgg ? easterEgg[currentDay].name : data.displayName;
    const lines = bestLineSplit(
      name
        .trim()
        .replace(/[\n\r\t]/g, "")
        .replace(/\s+/g, " "),
...
function drawDisplayNameTooltip(
  s = scaleFactor,
  x = width / 2,
  y = scaleFactor * 370
) {
